...
Unseen OrbitSweep / short SeedSweep; report freerun means or top ranks
...
`SeedSweep` / OrbitSweep / FreeRun (>0 overrides for that run; 0 keeps config).
...
on Parallel* / FreeRun* (`>0` overrides for that run). No dedicated A/B
campaign (`Campaign_SpectralRadiusAB` removed).
...
matched Unseen OrbitSweep / short SeedSweep; **or** a clear negative for each arm
