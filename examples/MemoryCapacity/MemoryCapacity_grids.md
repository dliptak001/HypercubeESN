# Memory Capacity — full grid archive

Archived TotalMC tables for the HypercubeESN linear memory-capacity diagnostic.
**Story, how to run, and at-a-glance peaks:** [MemoryCapacity.md](MemoryCapacity.md).

These tables are **not** auto-synced to whatever `main()` currently has checked
in. Single-seed peaks are **indicative**; use `RunSeedSurvey` (or Appendix B)
before locking an op-point for other tasks.

---

## Reference campaign protocol
...
| `input_scaling` | 0.06 |
| Reservoir seed | **47397376** (main grids + Appendix A) |
| Meter | warmup **4000** · collect **25000** · Kmax **4000** · ridge **1e-4** · train_frac **0.7** · `input_seed` **0xc0ffee** |

Every main-grid cell early-stopped (`e`); no open-tail undercount.
