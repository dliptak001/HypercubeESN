# Lorenz_JanusCursor — experiment tracking (Janus research harness)

Live result log for the Janus half-anchored free-run harness under
`Research Topics/Lorenz_JanusCursor/`. Protocol and scoring live in
[`README.md`](README.md). Knobs in [`Lorenz.h`](Lorenz.h) `config::`.
...
**Raw log:** `Research Topics/Lorenz_JanusCursor/Lorenz.exe-30-100_future_and_past.txt`
...
> default the **example** path (`examples/Lorenz/`) to forward-only (or a true
> unassisted free-run harness). **Janus is not discarded** — it lives under
> **`Research Topics/Lorenz_JanusCursor/`** for parameter-space exploration
> (past dose, lag curriculum, dual-drive stress, ablations,
> vanilla-ESN-under-Janus, etc.). Run 4/5 only shows past is not load-bearing
> for mean VPT/GS at *this* op-point; that is one point in a large space, not a
> verdict on Janus as a research program.

**To lock in (example only):** simplify `examples/Lorenz/` toward forward
free-run; keep Janus alive here under Research Topics — do not re-run A/B at
the same dose unless exploring a new point in param space.
...
### Research Topics archive + examples/Lorenz rework

**Intent:** `examples/Lorenz/` stays the **spotlight** public example (major
rework upcoming). `Research Topics/Lorenz_JanusCursor/` is a **quiet archive**
of Janus surveys/logs — not advertised in project/SDK docs.

- [x] Create **`Research Topics/Lorenz_JanusCursor/`** snapshot (copy of Janus
      harness + logs). Do not promote from root/SDK READMEs.
- [ ] **Major rework of `examples/Lorenz/`** — simple forward free-run storefront;
      CMake `Lorenz` target builds from `examples/Lorenz/` (spotlight).
- [ ] Research follow-ups stay in this archive (param sweeps, vanilla-under-Janus,
      etc.) without burdening example README.
...
- [ ] **Python MVP** (~1–1.5 day): NumPy vanilla ESN; Janus geometry
      ...
      small multi-seed table under **this** research folder (preferred home).
